...
class _InvoiceScreenState extends State<InvoiceScreen> {
  ...
  try {
    ...
    String fileName = 'invoice_${widget.service.id}_${DateTime.now().millisecondsSinceEpoch}.pdf';

    Directory? saveDir;

    if (Platform.isAndroid) {
      // Preferred way: direct path to public Downloads (works Android 10–15 without permission on API 33+)
      saveDir = Directory('/storage/emulated/0/Download');
      
      // Fallback / alternative (some devices / custom ROMs)
      // if (!await saveDir.exists()) {
      //   saveDir = Directory('/storage/emulated/0/Downloads');
      // }
    } else if (Platform.isIOS) {
      // iOS → usually saves to Files app → On My iPhone → Downloads (or use Files picker)
      saveDir = await getApplicationDocumentsDirectory(); // or use sharesheet
    } else {
    ...
    String displayPath = saveDir.path;
    if (Platform.isAndroid) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Invoice downloaded to $displayPath\n$fileName'),
        duration: const Duration(seconds: 5),
        action: SnackBarAction(
          label: 'Open',
          onPressed: () {
            // Optional: open_filex or url_launcher to open PDF
            // Example: OpenFile.open(filePath);
          },
        ),
      ),
    );
