    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['profile'] != null) {
          ...
          // 🔹 Update SharedPreferences from profile
...
await prefs.setString(
  ...
  return value
      .split(' ')                           // शब्दों में बाँटो (spaces से)
      .map((word) => word.trim())           // हर शब्द के extra spaces हटाओ
      .where((word) => word.isNotEmpty)     // खाली शब्द हटाओ
      .map((word) {
        if (word.isEmpty) return word;
        return word[0].toUpperCase() + word.substring(1).toLowerCase();
      })
      .join(' ');                           // दोबारा space से जोड़ दो
}
  ...
  Future<bool> updateProfile() async {
    ...
    try {
      ...
      if (firstnameController.text.trim().isNotEmpty) {
        firstnameController.text = _capitalizeWords(firstnameController.text.trim());
        request.fields['firstname'] = firstnameController.text.trim();
      }
      if (lastnameController.text.trim().isNotEmpty) {
        lastnameController.text  = _capitalizeWords(lastnameController.text.trim());
